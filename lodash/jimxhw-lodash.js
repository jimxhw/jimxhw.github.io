var jimxhw = {
	...
	isArrayLike: function (value) {
		return (
			!jimxhw.isFunction(value) && value !== undefined && value !== null && value.length >= 0 && value.length <= Number.MAX_SAFE_INTEGER
		);
	},
	...
	pullAllBy(array, values, iteratee = jimxhw.identity) {
		...
		return array.filter((it) => {
			return !values.some((value) => {
				return iteratee(value) === iteratee(it);
			});
		});
	},
	...
	zip: function (...arguments) {
		let maxLength = arguments.reduce((x, y) => {
		...
		return arguments[0].map((x, i) => {
			return arguments.map((j) => j[i]);
		});
	},
	...
	isNil: (value) => {
		return jimxhw.isNull(value) || jimxhw.isUndefined(value);
	},
