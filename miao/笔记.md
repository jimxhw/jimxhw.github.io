...
## 对象原型
  ...
  * 原型
      ...
      - 判断一个值的原型是谁就看其本身是什么类型，然后找到其类型的构造函数  
      - null ,undefined 没有原型
 ...
 * require函数  
  ...
  * requestAnimationFrame(function)
    ...
    回调函数function会被传入DOMHighResTimeStamp参数，DOMHighResTimeStamp指示当前回调函数被触发的时间
  
 ## 事件
   * 事件处理器
     浏览器和外部进行交互时触发的处理函数
   * element.addEvenListener(事件名，处理函数)  为元素添加事件处理器
   ...
   * Event 是一个全局事件对象，触发事件时该对象会传递给处理函数，浏览器同一时刻只有一个事件运行
     ...
     Event.type  返回事件名
     Event.keyCode  记录那个键被按下，返回对应的unicode码
     Event.Propagation 阻止事件传播到下一个元素（调用元素上后面的事件处理器还是会执行） 
     Event.ImmediatePropagation 阻止事件传播到下一个事件处理器（调用元素上后面的事件处理器不会会执行）
   ...
   * 传播 : 同一个事件触发了多个处理函数
     ...
     目标阶段： 最里面的触发事件元素 
     冒泡阶段： 由内到外，默认触发
     传播路径：由捕获阶段到目标阶段，目标阶段里面的处理函数按照代码顺序执行，不区分捕获和冒泡，之后是冒泡阶段    
