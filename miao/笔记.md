...
###  多列
   ...
   * break-inside:avoid-column; 使元素在一列中显示，不要左右断开显示
   * column-span：1/all;
...
### 2D变换
  ...
  * transform-origin：不动点，元素围绕改点进行变换，相当于变换坐标系的原点
    * 默认值为元素水平垂直的中点
    * 表达方式 
...
### 3D变换
  * transform-origin：x,y,z;  设置原点的位置
   ...
   * flex item相关属性
     ...
     * flex-shrink属性定义了项目的缩小比例，默认为1，即如果空间不足，该项目将缩小。 
       ...
       - item缩小的权重是 其宽度（高度）乘以flex-shrink的值 ，缩小的距离即为该item所占的权重比例乘以整体的缩小距离；
         eg: [(300*2)/（400*3+300*2+500*4)](权重 比例)*200(整体的缩小距离)
