...
## 开启 node 运行
  ...
  * 第一步 node  创建 node 运行环境
  * Node.js Buffer（缓冲区）  b = Buffer.alloc(8)   创建对象 b 有 8 个字节大小
  * b.write + tab 键 可以输出 write 函数类的方法；如 b.writeDoubleBE(Number.MAX_SAFE_INTEGER ,0) 可以看到 Number.MAX_SAFE_INTEGER 在 8 个字节里面的储存方式，第二个参数表示索引，默认为 0
...
## 整数在计算机中的表示和运算
  ...
  * 原码 1101        12568
    ...
    补码 0011        87432
    补码 = 周期 - 原码
    为了计算方面，通过周期 -1- 原码 +1 的方式计算补码（这样做减���不会借位，计算机是通过这样求反码的）
...
## HTTP
  ...
  * Ajax
    Asynchronous JavaScript + XML（异步JavaScript和XML）用来描述一种使用现有技术集合的‘新’方法
  * 常见响应状态码
...
## Node.js
  * 常用API
    ...
    * var queryParse = require('querystring') 调用node上面的'querystring'模块，可以解析http协议的query字符串数据
      queryParse.parse(query)  
  * 常用命令
    * nodemon xxx.js  以nodemon启动某js文件，服务器修改文件时客户端同步更新   
    * 