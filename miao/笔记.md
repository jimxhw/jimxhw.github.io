...
## CSS 编写规范
   ...
   * 外部引入的 css 文件样式不是储存在元素的 style 标签上面
...
## HTTP
  ...
  * 跨域问题
    ...
    * JSONP 一种跨域方案
        ...
        * 给路径传递参数 get 想要的资源，只支持get方法不支持post
        * 防止scrip标签加载的资源有问题
          * 给标签加一个integrity属性，值为正确资源的哈希值（md5或者sha1）
          * CSP内容安全策略
        * 原理
          * 动态生成一个scrip标签
          * 生成一个随机的callbackName
          * 在全局生成一个这样的callback函数
          * url为目的url加上序列化的参数，再加上 '&callback=' + callbackName
          * 将scrip标签的src属性等于拼接后的url
          * 将scrip标签加载到dom中 document.head.appendChild(script)
          * 页面会对指定地址资源进行请求
          * 服务端的处理是将指定位置的数据拿到，返回的content-type为text/javascript，返回结果为callbackName(data),直接会在全局调用；
...
# Node.js
  ...
  * 常用命令
    ...
    * __dirname 模块所在文件夹的绝对路径，全局变量  app.js 所在的路径
      __filename 模块本身目录的绝对路径  ，全局变量 app.js 所在的路径 /ap
  ...
  * 事件循环
    ...
    * process.nextTick
      process.nextTick(f)的执行优先级高于promise,会安排函数会在两个阶段之间执行
    * 事件循环原理
             ┌───────────────────────────┐
          ┌─>│           timers          │
          │  └─────────────┬─────────────┘
    ...
    - 首先把事件循环的逻辑初始化一下，然后执行初始的代码（node app.js里的脚本），这些代码的执行会在事件循环的几个阶段安排几个函数
    - 进入 timers 阶段
...
## webpack
  ...
  * 打包原理
    首先通过 loader 将资源转化为 js 模块，然后通过 require 函数拿到文件及其依赖的所有模块和路径的映射关系，打包到一个 js 文件里面
