...
- 常用 git 命令
  ...
  - git rm -r --cached 文件名 忽略文件 相当于写到 gitignore 里面
    git ignore 只会对不在 git 仓库中的文件进行忽略，如果这些文件已经在 git 仓库中，则不会忽略，需要使用这个命令
  - 回退到指定版本
    - git reset --hard bfa3a069a5738e7b657c4e31f7ef18f18fe47303
    - git push -f
...
* browse 同步
  ...
  - browser-sync start --server --directory -f "_.html,_.js" 安装同步 HTML 和 js
...
- <a href="#whoami">跳转本页的底部</a> <input id="whoami"> <!-- 指定跳转位置，id 为 whoami 的元素 -->
...
- 拿到 canvas 元素
  ...
  canvas.style.imageRendering = 'pixelated' 用于设置图像缩放算法，表示图像看着像是由大块像素组成的，相邻的地方不会模糊
...
- <i> 斜体，仅仅是字体的斜体
  <em> 斜体，语义的强调
  <b> 加粗，仅仅是字体的加粗
  <strong> 加粗，语义的强调，比 em 更强烈一点

# CSS 知识汇总

## CSS 编写规范

- CSS 属性 v .bat.adf:first-of-type 先寻找 div 后代中的 first-of-type ，再在筛选出的标签中寻找符合。bat.adf 的类标签
- :nth-c 选择器中， 冒号描述的是紧挨着其前面元素，如果前面紧挨着空格，则描述的是某元素的后代。如果前面紧挨着 class 或者 ID，先选择元素再在元素中寻找对应的类或者 ID。

  - dihild() 和 :nth-of-type() 的差别
...
- 在 CSS 中，两个或多个毗邻（父子元素或兄弟元素）的普通流中的块元素垂直方向上的 margin 会发生叠加。
...
- 下列方式会触发 BFC：
  ...
  display 值为 flow-root 的元，使用这个属性之后，该元素会生成一个块级容器框，并且使用的是流布局
...
- 层叠覆盖关系
  ...
  - z-index 必须配合定位使用
    - 调节层叠覆盖关系，常规流默认值为 0
    - z-index: 值； 值大的盖住值小的，可以 F 有负值，都为整数。
  - 常用布局
    - 元素明显重叠时，可用定位实现
  - 元素的方位
...
- 使用 rem 单位； 等比适配,算出设计稿和实际移动端宽度比，所有布局尺寸通过这个比值进行转换，通常把这个比值设置为 rem 的值
  rem 是根元素（通常为<html> 元素）的 font-size 大小
  1rem=100a
  设置 x*a=100vw;
...
- void 运算符 对给定的表达式进行求值，然后返回 undefined。 void 0 === undefined  
  在网站当中，通常会将 a 标签的 href 属性设置为"javascript:void(0)"，目的是：创建一个超链接，但是，在点内击链接时，既不能够跳转，也不会引起页面的刷新
...
function curry(f, length = f.length) {
	return function (...args) {
		if (args.length >= length) {
			...
			return curry(f.bind(this, ...args), length - args.length);
		}
	};
}
```
...
function compose(...fns) {
	return function (...args) {
		return fns.reduce((f1, f2) => {
			...
			return typeof f1 === 'function' ? f2(f1(...args)) : f2(f1);
		});
	};
}
...
- 可选链语法（Chrome 等大部分浏览器支持）
  obj?.abc 如果 obj 为 null 或者 undefined 不会直接报错，而是返回 undefined
...
- 构造函数 var f = new Function()
  ...
  - function myNew(Obj,...Arg){
    let obj = Object.create(Obj.prototype) /_ 创建一个对象 obj，obj 的原型是构造函数的原型属性 _/
    Obj.apply(obj,Arg) /_ 将构造函数的 this 绑定到 obj _/
    return obj/_ 返回 this _/
    }
...
- this
  ...
  - 以纯函数形式调用时，this 就是 window ，严格模式下是 undefined
...
- 箭头函数
  ...
  - 不能用 new 调用，没有 arguments 对象，没有重复参数
...
- XSS 跨域整个站点的脚步攻击 Cross Site Scripting
  ...
  - 实例
    - 用户打开了某支付网站
    - 马上用户又打开了病毒网站，在这个网站上面诱惑用户发送对支付网站的请求 http://zhifubao.com/withdraw?account=lyq&amount=10000&for=lyq2 这是一个转账请求
...
- 应用层 ：在传输层的数据协议之上的另一种协议（软件以应用层的协议解析其它软件发来的数据）
  ...
  - DNS：Domain Name System 将域名转化为 ip 地址的协议
    - 域名解析 根DNS服务器=>顶级DNS服务器(如解析以com结尾的服务器，权威DNS服务器(如qq.com)）
    - 包裹在 UDP 的数据里
    ...
    - DNS 劫持最直接的手段是通过入侵 DNS 服务器，获取攻击目标域名的解析记录控制权，之后修改攻击目标域名的解析结果
  - CDN Content Delivery Network，基于内容的分布式分发网络 https://juejin.cn/post/6844903873518239752

    - CDN 网络是在用户和服务器之间增加 Cache 层， 如何将用户的请求引导到 Cache 上获得源服务器的数据，主要是通过接管 DNS 实现，访问使用 CDN 缓存后的网站的过程
    - 原理流程
      - 网站 A 使用了 CDN 服务，向 A 请求资源时，DNS 服务器会根据 cnane 记录得到网站 B,网站 B 的 ip 地址是 CDN 全局负载均衡系统服务器
      - CDN 全局负载均衡系统服务器会根据用户位置，网络状况分配一个 CDN 缓存服务器 ip 地址返回
      - 客户端向缓存服务器发送请求，缓存服务器向真实的 ip 地址请求数据并缓存，返回数据给客户端完成数据服务过程
    - 通过 CND 全局负载均衡系统服务器进行二次 dns 解析
...
- cookie 相关的属性
  ...
  - SameSite 可以让 Cookie 在 ��� 站请求时不会被发送
...
- URL 编码
  ...
  - URL 会对一些特殊的字符进行转义编码，使用 %16 进 ������ 码方式
...
- 模块
  ...
  - 将一个基于回调的函数转化为一个返回一个 promise 的函数
    ...
    return function (...args) {
    return new Promise((resolve, reject) => {
    callbackBasedFunction(...args, (err, data) => { <!-- data 异步调用 args 后得到的结果 -->
    if (err) {
    reject(err)
    } else {
    resolve(data)
  ...
  - 生成器函数和 promise 结合替代异步调用 (async await 的原理）
    ...
    return new Promise((resolve, reject) => { <!-- async await 函数返回一个 promise -->
...
- State 的更新可能是异步的，出于性能考虑，React 可能会把多个 setState() 调用合并成一个调用。
  ...
  - 在异步函数里面调用 setState 那么 state 的更新是同步的,比如原生 js 绑定的事件，setTimeout/setInterval,Promise.then,async + await 等
...
- 协调，React 的 diff 函数，即前后虚拟 dom 对比方式
  ...
  - 对比方式
    ...
    - 有 key 时
      ...
      对比时 key 值不同，则 react 先销毁该组件（有状态组件的 componentWillUnmount 会执行），然后重新创建该组件
...
- Redux
  ...
  - 用法
    ...
    - var state = {
      ...
      if (mutation) {
      return mutation(state, action) 把 state 传给 produce((draft,action)=>{}) 这个函数，操作 state
      } else {
      return state
      }
      }, state)
  ...
  - 下层组件如何接入 store，需要第三方插件集成
    ...
    - connect 函数的用法，下层组件必须用 connect 函数包一层
      ...
      - mapStateToProps 函数，mapStatestate 接收 store 的 state; state=>{return {}}, 对 state 进行相关操作，返回一个对象 obj1
      - mapDispatchToProps 函数，mapDispatch 接收 store 的 dispatch; dispatch=>{return {deleteTodo: (idx) => dispatch({type: 'deleteTodo', idx})}}, 返回一个对象 obj2，对象 34 属性是组件相关方法名称，属性值是一个函数，这个函数里面会 dispatch 相关 action 到 store 的 reducer 函数，触发组件更新；
...
- 编译原理 https://www.codenong.com/p12180913/
