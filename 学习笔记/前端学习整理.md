...
- ![image-20250712214718642](/Users/xiahaowei/Library/Application Support/typora-user-images/image-20250712214718642.png)
...
- XML 一种数据格式，是一种包含了数据以及数据说明的文本格式规范,类似 JSON，现在基本不用
- SVG 是一种基于 XML 语法的图像格式。是属于对图像的形状描述，所以它本质上是文本文件，体积较小，且不管放大多少倍都不会失真。
- SVG 文件可以直接插入网页，成为 DOM 的一部分，可直接绑定事件。然后用 JavaScript 和 CSS 进行操作。SVG 文件还可以转为 BASE64 编码，然后作为 Data URI 写入网页。

## canvas

- 基于像素的位图绘图 API，通过 JavaScript 动态绘制图形，每次重绘都会重新渲染整个画布，适合绘制复杂的、高性能的动画和游戏
- 性能强
...
- 粘性定位 sticky
  粘性定位可以被认为是相对定位和固定定位的混合。元素在跨越特定值前为相对定位，之后为固定定位。在 viewport 视口滚动到元素 top 距离小于 设定值之前，元素为相对定位。之后，元素将固定在与顶部距离 10px 的位置，直到 viewport 视口回滚到阈值以下；当其包含块的文档区脱离视口时，它会被包含块带走。元素在常规流的位置保留
  - 导航栏、侧边栏等场景
- 层叠覆盖关系
  - 定位流盖住常规流
...
- 清除浮动：clear 属性，用于块元素，使其下移到两边没有浮动元素。块元素的 border box 和浮动元素的 marginbox 紧挨着；
...
- 伪元素的属性
  ...
  - content 属性，必要属性，表示伪元素的内容；它的值是文字或者空值，多个值之间用空格分开 content："" "asf" attr（父元素的属性）；
...
- pointer-events
  默认值为 auto，鼠标可以交互
  值为 none 时，鼠标在对象里面无法交互，鼠标会击透这个元素
...
- visibility，支持动画
  ...
  z-index: 也支持动画
- min-height/width max-height/width 设置元素宽高的最大最小数值。
...
- 好用的图标字体网站
  [https://www.iconfont.cn](https://www.iconfont.cn) 下载图片格式的图标
  [https://fontawesome.com/](https://fontawesome.com/) 下载 css 文件后引用
...
- line-break 处理如何断开（break lines）带有标点符号的中文、日文或韩文（CJK）文本的行。
  - 换行时由于中文标点符号不能出现在行头，可能导致上一行有空白
  - line-break :anywhere 可以解决上面的问题
...
- 使用 rem 单位； 等比适配,算出设计稿和实际移动端宽度比，所有布局尺寸通过这个比值进行转换，通常把这个比值设置为 rem 的值
  1rem=100a
  设置 x*a=100vw;
  x 为视觉稿的宽度（开发时设计人员给的值），100vw 是移动端的宽度，可以用 js 直接测出来，这样就可以算出 rem 的值，所有布局宽高都使用 rem 为单位；
...
- opacity ：0~1 透明度
  ...
  - 父元素透明度会影响子元素透明度，解决方法，不用 opacity，用 rgba 设置透明度
...
- animation 动画（可以简写，保证 duration 在 delay 前面即可）
  - @keyframes name {关键帧（10%）{属性：值} 关键帧（50%）{属性：值} 关键帧（100%）{属性：值}}
...
- 知识来源： [http://www.ruanyifeng.com/blog/2015/07/flex-grammar.html](http://www.ruanyifeng.com/blog/2015/07/flex-grammar.html)
...
- 更多知识 [http://www.ruanyifeng.com/blog/2012/06/sass.html](http://www.ruanyifeng.com/blog/2012/06/sass.html)
- sass 转化为 css 网站 [https://www.sassmeister.c`om/](https://www.sassmeister.c%60om/)
- sass 文件可以有 2 中扩展名
  - scss 可以嵌套的 css 语法
  - sass 可以嵌套的 css 语法，但是没有任何{}，靠缩进分类
- <link rel="stylesheet/scss" type="text/scss" href="styles.scss">
  <style type="text/scss">
- 类似于根一样的嵌套规则，@import 命令，用来插入外部文件。如 @import "path/filename.scss";
- & 符号表示父选择器，可直接使用，表示没有空格，和 less 一样；
- 变量 $ , 变量仅在它定义的选择器嵌套层级的范围内可用，加上！globle 就是全局属性；
  　　$blue : #1875e7;
  　　 div {
  　　　 color : $blue;
...
- 复合逻辑门
  ...
  - [https://zh.wikipedia.org/wiki/%E4%B8%8E%E9%9D%9E%E9%97%A8](https://zh.wikipedia.org/wiki/%E4%B8%8E%E9%9D%9E%E9%97%A8)
- 如果我们把相应的逻辑运算表达成逻辑函数，然后构造一个与逻辑函数对应的逻辑电路，则该电路即可表达我们的逻辑
- 由此我们可以使用电路实现任意的逻辑，实际上计算机电路中所有的逻辑门也都是用与非门来表达的
- A 或 B 取反是 非 A 且非 B A 且 B 取反是 非 A 或非 B
